'use strict';
var q = require('q'),
    log = require('debug')('gemini:limited-use-set'),
    _ = require('lodash'),
...
LimitedUseSet.prototype.clear = function() {
    ...
    return q.allSettled(this._objects.map(this._finalize)).then(function(results) {
        _this._objects = [];

        var rejected = _.filter(results, {state: 'rejected'});
        if (rejected.length) {
            return q.reject(new MetaError(_.map(rejected, 'reason')));
        }
    });
};
