...
function Gemini(config) {
    ...
    function extendRunnerArgs(paths, options) {
        if (!options) {
            options = paths;
    ...
    function getRunner(runnerInstance, paths, options) {
        return _this.readSuite(paths)
            .then(function(suite) {
                options.reporters.forEach(applyReporter.bind(null, runnerInstance));
                return runnerInstance.run(suite);
            });
    }
    ...
    this.gather = function(paths, options) {
        extendRunnerArgs(paths, options);
        return getRunner(new ScreenShooter(config), paths, options);
    };

    this.test = function(paths, options) {
        extendRunnerArgs(paths, options);
        return getRunner(
            new Tester(config, {tempDir: options.tempDir}),
            paths,
            options
        );
    };
