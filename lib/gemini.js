...
module.exports = class Gemini extends QEmitter {
    ...
    _run(stateProcessor, paths, options) {
        ...
        const envBrowsers = splitBrowsers(process.env.GEMINI_BROWSERS);
        const envSkipBrowsers = splitBrowsers(process.env.GEMINI_SKIP_BROWSERS);
        const browsers = options.browsers || envBrowsers;

        this.checkUnknownBrowsers(browsers);

        const validBrowsers = this.getValidBrowsers(browsers);
