...
exports.prepareScreenshot = (selectors, opts) => {
    opts = opts || {};
    try {
...
exports.resetZoom = () => {
    let meta = query.first('meta[name="viewport"]');
    if (!meta) {
...
function findIgnoreAreas(selectors) {
    let result = [];
    util.each(selectors, (selector) => {
        const element = query.first(selector);
        if (element) {
...
function adjustRect(rect, shadows, outline) {
    const shadowRect = calculateShadowRect(rect, shadows),
