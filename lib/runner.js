...
module.exports = inherit(EventEmitter, {
    ...
    _runStateInBrowser: function(state, browser, context) {
        ...
        if (state.shouldSkip(browser)) {
            _this.emit('skipState', suite.name, state.name, browser.name);
            return;
        }
        _this.emit('beginState', suite.name, state.name, browser.name);
        return browser.captureState(state, context)
            ...
            .fin(function() {
                _this.emit('endState', suite.name, state.name, browser.name);
            });
    },
