...
var TestsRunner = inherit(Runner, {
    ...
    _runTestSession: function(suites) {
        ...
        var _this = this;
        return sessionRunner.run(suites, this._captureProcessor)
            .then(function() {
    ...
    _prepare: function() {
        return this._captureProcessor.prepare(this);
    },
    ...
    _handleStateResult: function(result) {
        ...
        if (!this._failCollector.tryToSubmitCapture(result)) {
            this.emit(this._captureProcessor.getProcessedCaptureEventName(), result);
        }
    },
    ...
    createScreenUpdater: function(config, options) {
        return new TestsRunner(config, ScreenUpdater.create(config, options));
    }
});
