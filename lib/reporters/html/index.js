...
Handlebars.registerHelper('status', function() {
    ...
    return isWarn(this)? 'section_status_warning' : 'section_status_success';
});
...
Handlebars.registerHelper('reason', function() {
    var error = this.warning || this.error.originalError || this.error;
    return error.stack || error.message || error;
});

Handlebars.registerHelper('collapse', function() {
    return isOK(this) && !isWarn(this)? 'section_collapsed' : '';
});

function isWarn(section) {
...
module.exports = function htmlReporter(tester) {
    ...
    tester.on('endTest', function(result) {
        ...
        imageCopyQueue.then(function() {
            ...
            return q.all([
                ...
                result.saveDiffTo(path.resolve(REPORT_DIR, reportData.diffPath))
            ]);
        });
    });
    ...
    tester.on('warning', function(error) {
        var currentSuite = browserSuites[error.browserId],
            state = currentSuite.statesByName[error.stateName];
