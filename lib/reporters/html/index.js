...
function prepareViewData(runner, reporterOptions) {
    return new Promise((resolve) => {
        ...
        runner.on(Events.TEST_RESULT, (result) => {
            result.equal ? model.addSuccess(result, reporterOptions) : model.addFail(result);
        });
...
function prepareImages(runner, reporterOptions) {
    function handleTestResultEvent_(testResult) {
        let actions = [];

        if (!reporterOptions.failedOnly || !testResult.equal) {
            actions.push(
...
module.exports = function htmlReporter(runner, reporterOptions) {
    reporterOptions = reporterOptions || {};
    const generateReportPromise = Promise.all([
        prepareViewData(runner, reporterOptions),
        prepareImages(runner, reporterOptions)
    ])
        .spread(view.createHtml)
        .then((html) => view.save(html, reporterOptions.path))
        .then(() => logPathToHtmlReport(reporterOptions.path))
        .catch(logError);
