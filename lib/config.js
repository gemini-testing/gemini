...
var Config = module.exports = inherit({
    __constructor: function(configPath, configText, overrides) {
        ...
        if ('tolerance' in config) {
            ...
            if (config.tolerance > 1 || config.tolerance < 0) {
                throw new GeminiError('Field "tolerance" must be between 0 and 1');
            }
            this.tolerance = config.tolerance;
        } else {
            this.tolerance = Number.MIN_VALUE;
        }

        if ('diffColor' in config) {
            if (typeof config.diffColor !== 'string') {
                throw new GeminiError('Field "diffColor" must be string');
            }

            if (!/^#[0-9a-fA-F]{6}$/.test(config.diffColor)) {
                throw new GeminiError('Field "diffColor" must be hexadecimal color string (i.e. #ff0000)');
            }
            this.diffColor = config.diffColor;
        } else {
            this.diffColor = '#ff00ff';
        }
