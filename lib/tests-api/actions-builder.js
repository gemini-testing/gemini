...
module.exports = inherit({
    ...
    selectorExecuteJS: function(selector, callback) {
        if (!_.isFunction(callback)) {
            throw new TypeError('executeJS argument should be function');
        }

        selector = [].concat(selector).join(', ');

        this._pushAction(this.selectorExecuteJS, function selectorExecuteJS(browser) {
            return browser.execute(serializeFuncContext(callback, selector));
        });
        return this;
    },
...
function serializeFuncContext(func, ctx) {
    return `(${func.toString()}(document.querySelector('${ctx}')));`;
}
