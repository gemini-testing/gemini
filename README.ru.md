...
Некоторые поля настроек конфигурационного файла могут быть заменены опциями командной строки или переменными окружения в соответствии со следующими приоритетом:
* Опции командной строки имеют наивысший приоритет — они переопределяют переменные окружения и значения в конфигурационном файле; 
* Опции переменного окружения имеют второстепенный приоритет — они переопределяют значения в конфигурационном файле;
* Настройки конфигурационного файла имеют наименьший приоритет;
* Если не заданы опции командной строки, переменные окружения и значения полей в конфигурационном файле, то используются настройки по умолчанию.
...
* `gridUrl` (CLI: `--grid-url`, env: 'GEMINI_GRID_URL') — адрес Selenium Grid для создания скриншотов. Параметр обязателен, если тесты запускаются в каких-либо браузерах кроме `phantomjs`.
* `browsers` — список браузеров, используемых для тестирования. Все браузеры должны быть доступны в Silenium Grid.
    ...
    browsers:
      <browser-id>:
        browserName: <name>
        version: <version>
        # ... остальные возможности браузера задаются через <key>: <value>
    ```
...
* `projectRoot` — корневой каталог проекта. Относительно этого каталога будут разрешены все относительные
  пути в других настройках и опциях. По умолчанию — это каталог, в котором находится конфигурационный файл.
...
* `capabilities` — дополнительные хараетеристики [Selenium] (https://code.google.com/p/selenium/wiki/DesiredCapabilities) и [Sauce Labs] (https://docs.saucelabs.com/reference/test-configuration/) для использования во всех браузерах:
    ...
    Предусмотрена возможность задания любых характеристик за исключением `browserName` и `version` (вместо них спользуйте значения поля `browsers`) и `takesScreenshot` (по умолчанию всегда установлено `true`).

* `debug` (CLI: `--debug`, env: GEMINI_DEBUG) — включить debug вывод в терминал.
* `parallelLimit` — по умолчанию, `gemini` запускается во всех браузерах параллельно. Но иногда, когда используются облачные сервисы, такие как SauceLabs, у вас есть лимит на количество браузеров которые могут быть запущены одновременно. Используйте эту настройку для ограничения количества браузеров, которые могут быть запущены `gemini`  параллельно.
* `strictComparison` — по умолчанию, только заметные различия будут считаться ошибками. Включите эту настройку для отслеживания всех различий.
* `diffColor` — указание цвета, который используется для подсветки различий между изображениями. Задается через шестнадцатеричных формат RGB (`#RRGGBB`), маджента (`#FF00FF`) по умолчанию.
* `http.timeout` — таймаут запроса к Selenium Grid, мсек.
* `http.retries` — количество попыток отправки запроса к Selenium Grid.
* `http.retryDelay` — пауза перед повторной отправкой запроса к Selenium Grid, мсек.
* `coverage` (CLI: `--coverage`, env: GEMINI_COVERAGE) — установите `true` для включения экспериментальной функции отчета о тестовом покрытии. Отчет будет попадать в директорию `gemini-coverage`.
