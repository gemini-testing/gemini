...
* `gridUrl` - адрес Selenium Grid для создания скриншотов. Параметр обязателен, если тесты запускаются в каких-либо бразуерах кроме `phantomjs`.
...
Если версия не указана, будет использована любая доступная версия браузера с указанным именем.
   ...
   `- browser name` это сокращенный вариант записи `- name: browser name`
...
* `skip([browser])` - пропустить все тесты и вложенные наборы для всех браузеров; только для перечисленных браузеров; или  только для указанных версий браузеров.
    ...
    - `skip(browserName)` или `skip({name: browserName})` - пропустить тесты для всех версий указанного браузера;
    - `skip({name: browserName, version: browserVersion})` - пропустить тесты только для указанной версии браузера;
