...
В отличие от других подобных инструментов, [Gemini](https://github.com/bem/gemini) позволяет проверять отдельные блоки, а не всю страницу целиком. Эта возможность делает тесты более надежными и позволяет эффективно реагировать на изменения любой части веб-страницы.
...
* `browsers` - список браузеров, используемых для тестирования. Все браузеры должны быть доступны в Silenium Grid.
    ...
    Есть возможность использовать несколько версий одного и того же браузера (только в том случае, если эти версии доступны в вашем экземпляре Selenium Grid).
