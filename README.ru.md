...
```
* `after(callback(actions, find))` — используйте данную функцию для выполнения кода после последнего состояния. Данные аргументы функции обратного вызова одинаковы и для функций обратного вызова `capture` и `before`.
...
gemin.suite('parent', function(parent) {
    ...
    gemini.suite('second child', function(child) {
        ...
        gemini.suite('grandchild', function(grandchild) {
            //вложенные наборы могут иметь собственные внутренние наборы
    ...
    gemini.suite('third child', function(child) {
        //этот вложенный набор использует абсолютно другой URL и селектор элемента
...
#### Покрытие CSS-кода (экспериментальная функция)

Задайте опцию `--coverage` (или добавте соответствующее поле в конфигурационный файл) для создания отчета о тестовом покрытии. `gemini` проверит ваши CSS файлы и cообщит о CSS правилах которые хотя бы один раз были полностью задействованы в тесте (выделеты в отчете зеленым цветом), частично (желтым) или вовсе не были задействованы (красным).

### Параметры командной строки

* `--config`, `-c` — указание на конфигурационный файл.
* `--root-url`, `-r` — указание URL, заменяющее поле `rootUrl` в конфигурационном файле.
* `--grid-url`, `-g` — указание URL, заменяющее поле `gridUrl` в конфигурационном файле.
* `--grep PATTERN` — выполнить только те тестовые наборы, которые соответствуют шаблону заданного регулярного выражения.
* `--help` — вывести краткую справку.
* `--version` — показать версию.

### Shell completion

Для включения tab-completion в shell командах, добавьте `. <(gemini completion)` в ваш shell rc файл, например: `~/.bashrc` для bash или `~/.zshrc` для zsh.

## Programmatic API

Для использования `gemini` в ваших скриптах или плагинах систем сборки проектов вы можете использовать экспериментальный [programmatic API] (https://github.com/bem/gemini/blob/master/doc/programmatic-api.md).
